...
A compact finite-volume code for compressible astrophysical fluid dynamics. I
built it outward from one-dimensional Euler shock tubes into a second-order
two-dimensional solver, then used that solver for smooth Kelvin--Helmholtz and
Rayleigh--Taylor instabilities and a two-dimensional Sedov--Taylor blast. I
have kept the emphasis on measured accuracy, conservation, reproducibility,
and explicit limitations rather than feature count.

![Density, vorticity, and speed in the measured Kelvin--Helmholtz resolution study](figures/kelvin_helmholtz_fields.png)

This figure shows my recorded $64^2$, $96^2$, and $128^2$ calculations at the
dimensionless time `t = 1.5`.

## Equations

The code solves the compressible Euler equations, with an optional constant
gravity source in two dimensions. In one source-free dimension they are
